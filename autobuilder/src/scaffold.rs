...
fn write_ac_test(tests_dir: &Path, ac: &Value, slug: &str, target_kind: &str) -> Result<()> {
    ...
    let body = format!(
        "//! Acceptance test for {id} ({level}) — generated by `autobuilder scaffold`.\n\
         ...
         #[test]\n\
         fn {fn_name}() {{\n\
         \x20   // TODO(edit-agent): implement the test body that verifies the\n\
         \x20   // AC description above. Until implemented, this test fails so the\n\
         \x20   // iterate-and-prove loop sees a real signal.\n\
