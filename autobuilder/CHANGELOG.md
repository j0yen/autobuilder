...
**Refinement (same v0.8.1, pre-tag):** the initial fix keyed the exclusion on `entry.commit_sha == head_sha`, which undercounts — SKILL.md's own ship sequence commits an `agent: refresh intent card for <slug>` commit right after the version bump and before the gate runs, so by gate time `head_sha` is one or more non-version-touching commits past the actual bump (this PRD's own shipping of `f8fba79` → `1054daf` is a live example). `walk_tag_lineage` only records entries where `[package].version` actually changed, so the correct test is "is this the newest lineage entry", not "is this entry's commit literally HEAD". Re-keyed accordingly in `rollback.rs`; added `rollback_tag_ac13_head_past_untagged_bump_with_trailing_commits_no_gap.rs` reproducing the exact trailing-commits shape. `ac11`/`ac12`'s existing coverage (HEAD-is-only-gap passes; older-non-HEAD-gap still blocks) re-verified green under the new keying.

## v0.8.0 — 2026-09-08

PRD-rollback-mechanical-chains (v0.4.0, shipped 2026-09-08) classifies a same-path superseding commit chain as `mechanical(chain)` by dry-run-reverting only the chain's terminal (most recent) commit — non-terminal members are marked mechanical on that basis alone and excluded from `blocking_count`, per that PRD's own AC1 (`src/rollback.rs:486-488`, doc comment: "non-terminal members are never individually re-checked"). That PRD's own reviewer-agent receipt (`target/autobuilder/receipts/reviewer-agent.json`, `reviewed_at=2026-09-08T13:52:00Z`, `public_api_audit` + `counter_attack`) found this exploitable by construction: a chain where a non-terminal member deletes a path that a later member re-adds with different content can be independently non-revert-clean while the terminal commit still reverts cleanly, and the classifier has no check that would catch it — a caller reading "mechanical(chain)" as "safe to roll back to any point in this chain" is misled for that specific member. The finding is disclosed in-code, not hidden, and inert in every receipt observed so far (`mechanical_chain` has been 0 in every real repo checked to date), but it is a genuine, constructible gap, not a hypothetical one — the reviewer wrote a runnable test skeleton reproducing it. This PRD closes the gap: verify every non-terminal chain member's revert-cleanliness too (not just the terminal one), reporting per-member status in the receipt instead of inheriting the terminal commit's verdict wholesale.
