...
# --- Hard gates ---
# Gates may legitimately fail mid-iteration (the loop runner sees the metric
# delta, not the gate's exit code, to decide advance/revert). The harness
# must still emit metrics.json so the loop has a record of the failed
# iteration. AC5 of the metric-harness PRD codifies this invariant; this
# bash bootstrap mirrors it via `|| true`.
...
if [ -x "$HOME/.claude/skills/autobuilder/rules/audit-checks.sh" ]; then
  ...
  # jq on an empty file exits 0 with no output; ensure these stay numeric so
  # the final --argjson emit doesn't choke.
  BLOCKING=${BLOCKING:-0}
  ADVISORY=${ADVISORY:-0}
fi
