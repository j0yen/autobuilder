...
//!
//! Every receipt is a JSON object that ends up under
//! `target/autobuilder/receipts/` with a `receipt_digest` field bound to its
//! own canonicalized contents, plus a `captured_at` UTC timestamp.
//! Centralizing the digest + timestamp logic keeps every receipt in the gate
//! honest about what it attests to.
//!
//! Public surface (kept minimal):
//!
//! - [`write`] — stamp `receipt_digest`, write pretty-printed JSON bytes to a path.
//! - [`now_rfc3339`] — current UTC instant as an RFC3339 string for `captured_at`.
//! - [`canonical_json_bytes`] — sorted-keys byte encoding (exposed for tests).
//! - [`sort_keys`] — recursive object-key sort (exposed for tests).
//! - [`secs_to_rfc3339`] — UNIX seconds → RFC3339 UTC (exposed for tests).
//!
...
/// Canonical byte encoding of `value`: recursively sorted object keys,
/// arrays preserved in order.
///
/// Two JSON values that differ only in object-key insertion order produce
/// identical bytes from this function. Arrays are positional and reordering
