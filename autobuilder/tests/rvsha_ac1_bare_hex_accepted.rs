//! AC1 (P0, PRD-autobuilder-reviewer-intent-card-sha): given a reviewer
//! object whose `intent_card_sha` is the bare 64-hex digest of the current
//! intent card (no `sha256:` prefix — the exact shape the 2026-09-17
//! 15:54:42Z proof-lane incident produced), `finalize` succeeds and writes
//! the canonical `sha256:<hex>` form into the receipt.
...
/// Build a fresh git repo with an `agent/intent-card.json` and return
/// (repo dir, head sha, bare intent-card hex).
...
#[test]
fn rvsha_ac1_bare_hex_accepted() {
    ...
    assert!(
        ...
        "finalize must accept a bare-hex intent_card_sha; stdout={stdout} stderr={stderr}"
    );
    ...
    assert_eq!(
        ...
        "receipt must carry the canonical sha256:<hex> form regardless of the bare input"
    );
    assert_eq!(receipt["decision"], serde_json::json!("pass"));
}
