...
- **Stage 5 — `evolve`** ✓ — aggregator over proposals; emits `evolve-report-<date>.md` + `evolve-diff-<date>.patch`. Two auto-apply tracks: (a) append-only `Suggestion`s (default-on, write + git commit + fingerprint), (b) pure-addition `PatchSuggestion`s promoted from template-drift detection across ≥2 slugs (default-on, `patch --dry-run` guard + `patch -p0` + git commit + fingerprint). Mixed/deletion drift surfaces as a `TemplateDriftAdvisory` for manual review (direction-ambiguous). `--dry-run` skips both apply tracks.
